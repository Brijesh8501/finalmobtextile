...
$query = mysql_query("select * from company_deetaails where C_Name='".$new1."' ") or die(mysql_error());
...
$duplicate1 = mysql_num_rows($query1);
   if($duplicate1==0)
	...
	{
      $query2 = mysql_query("INSERT INTO `company_deetaails` (`Company_Id`, `C_Name`, `Address`,`cnt_id`,`ct_id`,`st_id`,`Phone_No`,`Mobile_No`,`Email_Id`,`status`,`Entry_Id`) VALUES (NULL, '$new1', '$Address','$cnt_id', '$ct_id', '$st_id','$Phone_No', '$Mobile_No', '$Email_Id', '$status', '$Entry_Id')") or die(mysql_error());
	  
	  $dateact = date('Y-m-d');
	$dateactfull = date('d-m-Y        h:i:s A');
	$Partact = "New Company Entry<br/>Company : ".$new1."<br/>Status : ".$status;
	$insactivity = "insert into activity(Activity_Id,Date,Particular,Type,Action,Datefull,Id) values(NULL,'$dateact','$Partact','Company - Insert','insert','$dateactfull','$Entry_Id')";
	mysql_query($insactivity); 
	
	   $insertGoTo = "companyprofilelistpage";
  ...
  echo '<script>window.location="'.$insertGoTo.'";</script>';
    ...
    {
      echo '<script>alert("This company is allready exists....");</script>';
    }
	}
}
?>
        ...
        <div id="div-1" class="accordion-body collapse in body">
            ...
            return false;">
                <div class="form-group">
                    ...
                    <div class="col-lg-8">
                      <input type="text" id="Company_Id" placeholder="" value="<?php echo getNewCompanyID(); ?>" class="form-control"  name="Company_Id" readonly />
                    </div>
                </div>
                <div class="form-group">
                    <label for="text2" class="control-label col-lg-4">Company :</label>
                    <div class="col-lg-8">
                      <input type="text" id="C_Name" placeholder="company name is not changeable afterwards so enter appropriate name" name="C_Name" value="<?php echo $new1; ?>" class="form-control" />
...
<div class="form-group">
                    ...
                    <div class="col-lg-8">
                    <input type="email" id="Email_Id" name="Email_Id" value="<?php echo $Email_Id; ?>" placeholder="Email ID" class="form-control" required/>
                   </div>
                </div>
                    <div class="form-group">
...
<?php
function getNewCompanyID()
{
	include("databaseconnect.php");
	$sql="select max(Company_Id)+1 from company_deetaails";
