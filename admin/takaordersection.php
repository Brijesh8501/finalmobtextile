...
if(isset($_REQUEST['action']))
{
	...
	$urls = explode(" ",$turl);
	$T_Order_Id = $urls[1];
	$sql = "select * from taka_order_master where T_Order_Id = '".$T_Order_Id."' ";
	...
	$sel_check_entry = "select sum(Amount) as sum_amount from taka_order_detailsorg where T_Order_Id='$T_Order_Id'";
...
else
{
	...
	$sql= "INSERT INTO  `taka_order_master` (`T_Order_Id` ,`Om_Date` ,`Customer_Id` ,`Broker_Id`,`Total_Amt`,`Discount`,`Grandtotal` ,`Advance_Amt`,`Delivery_Date`,`Remark`,`Status`,`Re_Amt`,`Entry_Id`)
VALUES (NULL , '$Om_Date' ,  '$t2',  '$t3',  '$Total_Amt',  '$Discount',  '$Grandtotal',  '$Advance_Amt',  '$Delivery_Date',  '$Remark',  '$Status',  '$Re_Amt', '$Entry_Id')";
$result = mysql_query($sql);
$sel_sub = "select * from taka_order_details where taka_order_details.T_Order_Id = '$T_Order_Id'";
$sel_exe = mysql_query($sel_sub);
while($sel_fetch = mysql_fetch_array($sel_exe))
{
	$ins_sub = "insert into taka_order_detailsorg (T_Od_Id,T_Order_Id,Quality_Id,Quantity,Rate,Amount,R_Id) values(NULL,'".$sel_fetch[1]."','".$sel_fetch[2]."','".$sel_fetch[3]."','".$sel_fetch[4]."','".$sel_fetch[5]."','".$sel_fetch[6]."')";
	mysql_query($ins_sub);
}
$del_sub = "delete from taka_order_details where taka_order_details.T_Order_Id = '$T_Order_Id'";
$del_exe = mysql_query($del_sub);
$sel_sub_or = "select * from taka_order_detailsorg where taka_order_detailsorg.T_Order_Id = '$T_Order_Id'";
...
else
{
 ...
 $insertGoTo = "taka_order_listpage?msg_error=$msg_error"; 
...
{  
	...
	$Re_Amt = $_REQUEST['Re_Amt'];
	$Entry_Id = $row_result['Registration_Id'];
	$sql1="UPDATE `taka_order_master` SET `Om_Date` = '$Om_Date', `Customer_Id` = '$t2', `Broker_Id` = '$t3', `Total_Amt` = '$Total_Amt', `Discount` = '$Discount', `Grandtotal` = '$Grandtotal', `Advance_Amt` = '$Advance_Amt', `Delivery_Date` = '$Delivery_Date', `Remark` = '$Remark', `Status` = '$Status', `Re_Amt` = '$Re_Amt', `Entry_Id` = '$Entry_Id' WHERE `taka_order_master`.`T_Order_Id` = '".$T_Order_Id."' ";
	$result1 = mysql_query($sql1);
	$updateGoTo = "taka_order_listpage?msg";
...
<?php include("sidemenu.php"); ?>
                <div class="inner">
                    <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header" align="center">TAKA ORDER</h1>
                </div>
            </div>
<div class="row">
<div class="col-lg-12">
...
?>
                    ...
                    <div class="col-lg-7">
                    <input type="text" id="T_Order_Id" placeholder="" class="form-control" value="<?php echo $T_Order_Id; ?>" name="T_Order_Id" readonly />
                    </div>
                </div>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-7">
                    <input type="text" id="Quantity" placeholder="Meters" class="form-control" name="Quantity" onkeypress="return isNumberKey(event)"/>
						 ...
						 if($days_remaining<=0)
{
	echo "<strong style='color:#F00;'>* Please renew your website</strong>";
							...
							?>
                            <a href="Taka_order_listpage"><input type="button" value=" Back" class="btn btn-inverse btn-lg " /></a>
                            <?php
							if($action=='update')
                             {
							?>
...
$(document).ready(function(){
   <?php if($action=='update'){?>
	$("#customer").val("<?php echo $t2; ?>");
		...
		sql="?T_Order_Id=<?php echo $T_Order_Id; ?>&action=<?php echo $action;?>";
		$("#table").load("takaordertable.php"+sql);
		});
		<?php include("shortcutkeys.php");?>
   </SCRIPT>
   <script src="assets/js/takaordersection.js"></script>
...
{
	include("databaseconnect.php");
	$sql="select max(T_Order_Id)+1 from taka_order_master";
	$result= mysql_query($sql);
	$row= mysql_fetch_array($result);
	if($row != null && $row[0] > 0)
		{
