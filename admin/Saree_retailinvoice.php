...
if(isset($_REQUEST['printinvoice']))
{
	...
	$Saree_Invoice_Id = $urls[1];
		...
		$sql = "SELECT saree_invoice.Saree_Invoice_Id, saree_invoice.Saree_Invoice_Date, saree_invoice.Saree_D_C_Id, saree_invoice.Saree_D_C_Date, customer_details.Cu_En_Name, customer_details.Address, broker_details1.B_Name, saree_invoice.Total_Amt, saree_invoice.Discount,saree_invoice.Interest, saree_invoice.Grandtotal FROM saree_invoice, customer_details, broker_details1 WHERE customer_details.Customer_Id = saree_invoice.Customer_Id AND broker_details1.Broker_Id = saree_invoice.Broker_Id AND Saree_Invoice_Id = '".$Saree_Invoice_Id."' ";
  ...
  <tr>
    <td colspan="3"><table width="100%" border="1" cellspacing="2" cellpadding="2" style="border-color:#000" class="table">
        ...
        <td colspan="3"><table width="100%" border="0" cellspacing="2" cellpadding="2">
         <tr>
          ...
          <tr>
            <td>Invoice No</td>
            <td>:</td>
            <td>&nbsp;<strong><?php echo $Saree_Invoice_Id; ?></strong></td>
          </tr>
          <tr>
            <td>Invoice Date</td>
            <td>:</td>
            <td>&nbsp;<strong><?php echo $Saree_Invoice_Date; ?></strong></td>
          </tr>
          <tr>
            <td>Challan No</td>
            <td>:</td>
            <td>&nbsp;<?php echo $Saree_D_C_Id; ?></td>
          </tr>
          <tr>
            <td>Challan Date</td>
            <td>:</td>
            <td>&nbsp;<?php echo $Saree_D_C_Date; ?></td>
          </tr>
          <tr>
            <td>Broker</td>
            <td>:</td>
      ...
      <tr bgcolor="#FFFFFF" style="color:#000">
        ...
        <th width="15%">Total Meters</th>
        <th width="15%">Total Weight</th>
        <th width="15%">Total Piecess</th>
        <th width="10%">Rate</th>
        <th width="15%">Amount</th>
      </tr>
       <tr>
      <td colspan="6" class="nobordersheight" valign="top"><table width="100%" border="0">
      <?php do { ?>
        <tr style="background-color:#FFF ; color:#000">
          <td align="left" width="25%"><?php echo $row_RsTakaInvoice['Quality_Name']; ?> <br />(<?php echo $row_RsTakaInvoice['Design']; ?>)<br />----------------------------------------------</td>
          <td align="left" width="15%">&nbsp;&nbsp;<?php echo moneyFormatIndia($row_RsTakaInvoice['Total_Meter']); ?></td>
          <td align="left" width="15%">&nbsp;&nbsp;<?php echo moneyFormatIndia($row_RsTakaInvoice['Total_Weight']); ?></td>
          <td align="left" width="15%">&nbsp;&nbsp;<?php echo moneyFormatIndia1($row_RsTakaInvoice['Total_Pieces']); ?></td>
          <td align="left" width="10%">&nbsp;&nbsp;<?php echo moneyFormatIndia($row_RsTakaInvoice['Rate']); ?></td>
      ...
      <tr style="color:#000; background-color:#FFF">
        <td colspan="4"  <?php if($Interest>=1) { ?>rowspan="5"<?php } else {?>rowspan="4"<?php } ?>><table width="100%" border="0" cellspacing="2" cellpadding="2">
           <tr>
