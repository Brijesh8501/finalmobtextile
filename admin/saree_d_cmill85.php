...
if(isset($_REQUEST['action']))
{
 ...
 if($action == 'update')
	...
	$urls = explode(" ",$turl);
	$Saree_D_C_Id = $urls[1];
	$sql = "select * from saree_d_mill where saree_d_mill.Saree_D_C_Id = '".$Saree_D_C_Id."'  ";
	$result = mysql_query($sql);
	$row = mysql_fetch_array($result);
	$Saree_D_C_Id = $row['Saree_D_C_Id'];
	$Saree_D_C_Date = $row['Saree_D_C_Date'];
	$Total_Lot = $row['Total_Lot'];
	$Saree_Mill = $row['Saree_Mill'];
	$Entry_Id = $row['Entry_Id'];
	$Entry_Date = $row['Entry_Date'];
	$Status = $row['Status'];
	date_default_timezone_set('Asia/Calcutta');
	 include("webrenew.php");
	$sel_check_entry = "select count(Saree_Lot_Id) as count_saree from saree_dcorgmill where Saree_D_C_Id='$Saree_D_C_Id'";
	...
	{
		$msg_check_saree ='<strong style="color:#F00;">'.'Please update this mill challan which is required</strong>';
	}}}
...
{ } else{   
   ...
   $Saree_Mill = strtoupper($_REQUEST['Saree_Mill']);
	$Status = $_REQUEST['Status'];
	$Entry_Id = $row_result['Registration_Id'];
	date_default_timezone_set('Asia/Calcutta');
	$Entry_Date = date('Y-m-d      h:i:s a');
	$sql= "INSERT INTO  `saree_d_mill` (`Saree_D_C_Id` ,`Saree_D_C_Date` ,`Saree_Mill`,`Total_Lot`,`Entry_Date`,`Entry_Id`,`Status`)VALUES (NULL , '$Saree_D_C_Date',  '$Saree_Mill', '$Total_Lot', '$Entry_Date', '$Entry_Id', '$Status')";
$result = mysql_query($sql);
$sel_sub = "select * from saree_d_c_2mill where saree_d_c_2mill.Saree_D_C_Id = '$Saree_D_C_Id'";
$sel_exe = mysql_query($sel_sub);
while($sel_fetch = mysql_fetch_array($sel_exe))
{
	$ins_sub = "insert into saree_dcorgmill (Sa_D_C_Id,Saree_D_C_Id,Saree_Lot_Id,Saree_Lot_Meter,Saree_Weight,Saree_Pieces,Design_Id,R_Id) values(NULL,'".$sel_fetch[1]."','".$sel_fetch[2]."','".$sel_fetch[3]."','".$sel_fetch[4]."','".$sel_fetch[5]."','".$sel_fetch[6]."','".$sel_fetch[7]."')";
	mysql_query($ins_sub);
}
$del_sub = "delete from saree_d_c_2mill where saree_d_c_2mill.Saree_D_C_Id='$Saree_D_C_Id'";
$del_exe = mysql_query($del_sub);
$sel_sub_or = "select * from saree_dcorgmill where saree_dcorgmill.Saree_D_C_Id = '$Saree_D_C_Id'";
...
else
{
 ...
 $insertGoTo = "saree_d_c_milllist?msg_error=$msg_error";
...
{ } else{
	...
	$sql= "UPDATE `saree_d_mill` SET `Saree_D_C_Date` = '$Saree_D_C_Date', `Saree_Mill` = '$Saree_Mill', `Status` = '$Status', `Entry_Date` = '$Entry_Date', `Total_Lot` = '$Total_Lot', `Entry_Id` = '$Entry_Id' WHERE `saree_d_mill`.`Saree_D_C_Id` = '".$Saree_D_C_Id."' ";
$result = mysql_query($sql);
 $updateGoTo = "saree_d_c_milllist?msg";
...
<body>
       <?php include("sidemenu.php") ?>
                <div class="inner">
                    <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header" align="center">MILL-SAREE CHALLAN</h1>
                </div>
            </div>
<div class="row">
<div class="col-lg-12">
        ...
        <div id="div-1" class="accordion-body collapse in body">
            ...
            <div class="col-lg-4">
             <div class="form-group">
                    ...
                    <div class="col-lg-6">
                        <textarea rows="3" cols="20" id="textarea1" name="Saree_Mill" class="form-control" ><?php echo $Saree_Mill;?></textarea><span id="error1" style="color:#F00";></span>
                    </div>
                </div>      
                        </div>
                       <div class="col-lg-4">
						  ...
						  if($days_remaining<=0)
{
	echo "<strong style='color:#F00;'>* Please renew your website</strong>";
							...
							?>
                        <a href="saree_d_c_milllist"><input type="button" value="Back" class="btn btn-inverse btn-lg " name="back"/></a> 
                             <?php
							if($action=="update" )
                             {
							?>
...
<script src="assets/js/sareemillchallan.js"></script>
...
function getNewChallanID()
{
	...
	$sql = "select max(Saree_D_C_Id)+1 from saree_d_mill";
	$result = mysql_query($sql);
	$row = mysql_fetch_array($result);
	if($row != null && $row[0] > 0)
		{
