...
$duplicate1 = mysql_num_rows($query1);
   if($duplicate1==0)
	...
	{
  $updateSQL = sprintf("UPDATE broker_details1 SET B_Name=%s, Address=%s, ct_id=%s, st_id=%s, Mobile_No=%s, Email_Id=%s, Status=%s, Entry_Id='$Entry_Id' WHERE Broker_Id=%s",
  ...
  $Result1 = mysql_query($updateSQL, $brijesh8510) or die(mysql_error());
   ...
   $dateact = date('Y-m-d');
	...
	$B_Nameactivity = strtoupper($_REQUEST['B_Name']);
	$statusactivity = $_REQUEST['Status'];
	$Partact = "Broker Entry<br/>Broker : ".$B_Nameactivity."<br/>Status : ".$statusactivity;
	$insactivity = "insert into activity(Activity_Id,Date,Particular,Type,Action,Datefull,Id) values(NULL,'$dateact','$Partact','Broker - Update','update','$dateactfull','$Entry_Id')";
	mysql_query($insactivity);  
	
  $updateGoTo = "brokerprofilelistpage";
...
mysql_select_db($database_brijesh8510, $brijesh8510);
$query_Recordset1 = sprintf("SELECT broker_details1.Broker_Id, broker_details1.B_Name, broker_details1.Address, city1.ct_id, city1.ct_name, country1.cnt_id, country1.cnt_name, `state1`.st_id, `state1`.st_name, broker_details1.Mobile_No, broker_details1.Email_Id, broker_details1.Status FROM broker_details1, city1, country1, `state1` WHERE city1.ct_id = broker_details1.ct_id AND country1.cnt_id = broker_details1.cnt_id AND `state1`.st_id = broker_details1.st_id AND Broker_Id = %s", GetSQLValueString($colname_Recordset1, "int"));
        ...
        <div id="div-1" class="accordion-body collapse in body">
            ...
            return false;">
                <div class="form-group">
                    ...
                    <div class="col-lg-8">
                      <input type="text" id="Broker_Id" name="Broker_Id" placeholder="" class="form-control" value="<?php echo $row_Recordset1['Broker_Id']; ?>" readonly />
                    </div>
                </div>
                <div class="form-group">
...
<div class="form-group">
                    ...
                    <div class="col-lg-8">
                    <input type="email" id="text4" name="Email_Id" placeholder="Email ID" class="form-control"value="<?php echo htmlentities($row_Recordset1['Email_Id'], ENT_COMPAT, 'UTF-8'); ?>" required/>
                   </div>
                </div>
                    <div class="form-group">
