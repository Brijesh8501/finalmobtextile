...
$query_rsDCSA = "SELECT saree_d_mill.Saree_D_C_Id, saree_d_mill.Saree_D_C_Date,saree_d_mill.Saree_Mill,Status, saree_d_mill.Entry_Date, saree_d_mill.Total_Lot, saree_d_mill.Entry_Id FROM saree_d_mill";
...
<body>
            ...
            <div class="inner">
                <div class="row">
                    <div class="col-lg-12">
                        <h2 align="center">MILL-SAREE CHALLAN PROFILE</h2>
                    </div>
                </div>
                <hr />
                <div class="row">
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading"><table border="0"><tr><td><a href="saree_d_cmill85?action=insert" target="_blank"><button class="btn btn-primary"><i class="icon-plus-sign icon-white"></i> Add Entry</button></a></td><td width="100%"><?php if(isset($_REQUEST['msg_error']))
...
{ echo $msg; } ?></td></tr></table></div>
  ...
  <tr class="odd gradeX" id="<?php echo $row_rsDCSA['Saree_D_C_Id']; ?>">
    ...
    <td><?php echo $row_rsDCSA['Saree_Mill']; ?></td>
    <td><?php echo $row_rsDCSA['Total_Lot']; ?></td>
     <td><?php echo $row_rsDCSA['Status']; ?></td>
    <td><?php echo $row_rsDCSA['Entry_Date']; ?></td>
   ...
   $strshuffle = str_shuffle($encodedUrl); 
    ?> <div class="tooltip-demo">
     <a href="saree_d_c_millview?view&Saree_D_C_Id=<?php echo $encodedUrl.''.$encodedUrl.''.$strshuffle; ?>" target="_blank"><button class="btn btn-inverse btn-xs" data-toggle="tooltip" data-placement="top" title="View" name="View<?php echo $row_rsDCSA['Saree_D_C_Id']; ?>" id="View<?php echo $row_rsDCSA['Saree_D_C_Id']; ?>"><i class="icon-eye-open"></i></button></a>&nbsp;
    <a href="saree_d_cmill85?action=update&Saree_D_C_Id=<?php echo $encodedUrl.''.$encodedUrl.''.$strshuffle; ?>" target="_blank"><button class="btn btn-inverse btn-xs" data-toggle="tooltip" data-placement="top" title="Update challan" name="Update<?php echo $row_rsDCSA['Saree_D_C_Id']; ?>" id="Update<?php echo $row_rsDCSA['Saree_D_C_Id']; ?>"><i class="icon-edit"></i></button></a>&nbsp;
    <a href="sarees_d_cmill?print&Saree_D_C_Id=<?php echo $encodedUrl.''.$encodedUrl.''.$strshuffle; ?>" target="_blank"><button class="btn btn-inverse btn-xs" data-toggle="tooltip" data-placement="top" title="Print challan" name="Print<?php echo $row_rsDCSA['Saree_D_C_Id']; ?>" id="Print<?php echo $row_rsDCSA['Saree_D_C_Id']; ?>"><i class="icon-print"></i> </button></a>&nbsp;
   ...
   <?php include("footer.php"); ?>
    ...
    <script>
	 $(document).ready(function () {
       ...
       $.post('saree_d_c_milllistdelete.php', {delete_id:del_id}, function(data) {
          if(data == 'true') {
