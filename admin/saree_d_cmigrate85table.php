...
if(isset($_SESSION['User']))
{
	...
	elseif($Design_Id<=0)
		...
		if($action=='insert')
             {
	$query = mysql_query("select * from saree_d_c_2migrate where Saree_Lot_Id='".$Saree_Lot_Id."' ") or die(mysql_error());
	$res_Fetch = mysql_fetch_array($query);
	$duplicate = mysql_num_rows($query);
   if($duplicate==0)
    {	
	$query4 = mysql_query("select * from saree_dcorgmigrate where Saree_Lot_Id='".$Saree_Lot_Id."' ") or die(mysql_error());
	$res_Fetch4 = mysql_fetch_array($query4);
	$duplicate4 = mysql_num_rows($query4);
	$res_Saree_D_C_Id4 = $res_Fetch4['Saree_D_C_Id'];
   if($duplicate4==0)
    {	
	$sql= "INSERT INTO  `saree_d_c_2migrate` (`Sa_D_C_Id` ,`Saree_D_C_Id` ,`Saree_Lot_Id` ,`Saree_Lot_Meter` ,`Saree_Weight` ,`Saree_Pieces` ,`Design_Id` ,`R_Id`)VALUES (NULL , '$Saree_D_C_Id' ,  '$Saree_Lot_Id',  '$Saree_Lot_Meter', '$Saree_Weight',  '$Saree_Pieces', '$Design_Id', '$R_Id')";
...
{
$query = mysql_query("select * from saree_dcorgmigrate where Saree_Lot_Id='".$Saree_Lot_Id."' ") or die(mysql_error());
	$res_Fetch = mysql_fetch_array($query);
	$duplicate = mysql_num_rows($query);
	$res_Saree_D_C_Id = $res_Fetch['Saree_D_C_Id'];
   if($duplicate==0)
    {	
	$sql= "INSERT INTO  `saree_dcorgmigrate` (`Sa_D_C_Id` ,`Saree_D_C_Id` ,`Saree_Lot_Id` ,`Saree_Lot_Meter` ,`Saree_Weight` ,`Saree_Pieces` ,`Design_Id` ,`R_Id`)VALUES (NULL , '$Saree_D_C_Id' ,  '$Saree_Lot_Id',  '$Saree_Lot_Meter', '$Saree_Weight',  '$Saree_Pieces', '$Design_Id', '$R_Id')";
$result = mysql_query($sql);
	}
	else
	{
...
$query_rsSaree = "SELECT saree_d_c_2migrate.Sa_D_C_Id, saree_d_c_2migrate.Saree_D_C_Id, saree_d_c_2migrate.Saree_Lot_Id, saree_d_c_2migrate.Saree_Pieces,saree_d_c_2migrate.Saree_Lot_Meter, saree_d_c_2migrate.Saree_Weight, quality_details.Quality_Name, design_details.Design, saree_d_c_2migrate.R_Id FROM saree_d_c_2migrate, quality_details, design_details WHERE design_details.Quality_Id = quality_details.Quality_Id AND saree_d_c_2migrate.Design_Id = design_details.Design_Id AND saree_d_c_2migrate.Saree_D_C_Id='".$Saree_D_C_Id."' ";
...
{
	mysql_select_db($database_brijesh8510, $brijesh8510);
$query_rsSaree = "SELECT saree_dcorgmigrate.Sa_D_C_Id, saree_dcorgmigrate.Saree_D_C_Id, saree_dcorgmigrate.Saree_Lot_Id, saree_dcorgmigrate.Saree_Pieces,saree_dcorgmigrate.Saree_Lot_Meter, saree_dcorgmigrate.Saree_Weight, quality_details.Quality_Name, design_details.Design, saree_dcorgmigrate.R_Id FROM saree_dcorgmigrate, quality_details, design_details WHERE design_details.Quality_Id = quality_details.Quality_Id AND saree_dcorgmigrate.Design_Id = design_details.Design_Id AND saree_dcorgmigrate.Saree_D_C_Id='".$Saree_D_C_Id."' ";
$rsSaree = mysql_query($query_rsSaree, $brijesh8510) or die(mysql_error());
$row_rsSaree = mysql_fetch_assoc($rsSaree);
$totalRows_rsSaree = mysql_num_rows($rsSaree);
}
	...
	<?php if($action=='insert')
{
    ...
    $('.btn-danger').click(function(){
       ...
       $.post('saree_d_c_migratetabledelete.php?del=ins', {delete_id:del_id}, function(data) {
          if(data == 'true') {
		  $('#'+del_id).remove();
			alert("Deleted!");
			Count();
...
elseif($action=='update')
{
	...
	$('.btn-danger').click(function(){
       var del_id = $(this).attr('rel');
       $.post('saree_d_c_migratetabledelete.php?del=upd', {delete_id:del_id}, function(data) {
          if(data == 'true') {
		  $('#'+del_id).remove();
			alert("Deleted!");
			Count();
