...
if(isset($_SESSION['User']))
{
	...
	elseif($Quality_Id<=0)
		...
		else
		{
			if($action=='insert')
             {
	   $query = mysql_query("select * from taka_dcorgmigrate WHERE taka_dcorgmigrate.Taka_Id='".$Taka_Id."' ") or die(mysql_error());
	   $duplicate_fetch = mysql_fetch_array($query);
$duplicate = mysql_num_rows($query);
$duplicate_taka = $duplicate_fetch['Taka_D_C_Id'];
   if($duplicate==0)
    {
		$query3 = mysql_query("select * from taka_d_c_2migrate1 WHERE taka_d_c_2migrate1.Taka_Id='".$Taka_Id."' ") or die(mysql_error());
	  $duplicate3 = mysql_num_rows($query3);
   if($duplicate3==0)
    {
	$sql= "INSERT INTO  `taka_d_c_2migrate1` (`Ta_D_C_Id` ,`Taka_D_C_Id` ,`Taka_Id` ,`Taka_Meter` ,`Taka_Weight` ,`Quality_Id` ,`R_Id`)VALUES (NULL , '$Taka_D_C_Id' ,  '$Taka_Id',  '$Taka_Meter',  '$Taka_Weight', '$Quality_Id',  '$R_Id')";
...
$duplicate_taka = $duplicate_fetch['Taka_D_C_Id'];
   if($duplicate==0)
    {
	$sql= "INSERT INTO  `taka_dcorgmigrate` (`Ta_D_C_Id` ,`Taka_D_C_Id` ,`Taka_Id` ,`Taka_Meter` ,`Taka_Weight` ,`Quality_Id` ,`R_Id`)VALUES (NULL , '$Taka_D_C_Id' ,  '$Taka_Id',  '$Taka_Meter',  '$Taka_Weight', '$Quality_Id', '$R_Id')";
$result = mysql_query($sql);
	}
	else
	{
...
$query_Recordset1 = "SELECT taka_d_c_2migrate1.Ta_D_C_Id, taka_d_c_2migrate1.Taka_Id, taka_d_c_2migrate1.Taka_Meter, taka_d_c_2migrate1.Taka_Weight, quality_details.Quality_Name,taka_d_c_2migrate1.Taka_D_C_Id, taka_d_c_2migrate1.R_Id FROM taka_d_c_2migrate1,quality_details WHERE quality_details.Quality_Id = taka_d_c_2migrate1.Quality_Id AND taka_d_c_2migrate1.Taka_D_C_Id='".$Taka_D_C_Id."' ";
...
{
	mysql_select_db($database_brijesh8510, $brijesh8510);
$query_Recordset1 = "SELECT taka_dcorgmigrate.Ta_D_C_Id, taka_dcorgmigrate.Taka_Id, taka_dcorgmigrate.Taka_Meter, taka_dcorgmigrate.Taka_Weight, quality_details.Quality_Name, taka_dcorgmigrate.Taka_D_C_Id, taka_dcorgmigrate.R_Id FROM taka_dcorgmigrate,quality_details WHERE quality_details.Quality_Id = taka_dcorgmigrate.Quality_Id AND taka_dcorgmigrate.Taka_D_C_Id='".$Taka_D_C_Id."' ";
$Recordset1 = mysql_query($query_Recordset1, $brijesh8510) or die(mysql_error());
$row_Recordset1 = mysql_fetch_assoc($Recordset1);
$totalRows_Recordset1 = mysql_num_rows($Recordset1);
}
	...
	<?php if($action=='insert')
{
 ...
 $('.btn-danger').click(function(){
       ...
       $.post('taka_d_cmigratetabledelete.php?del=ins', {delete_id:del_id}, function(data) {
          if(data == 'true') {
	        $('#'+del_id).remove();
			alert('Deleted');
			Count();
...
elseif($action=='update')
{
	...
	$('.btn-danger').click(function(){
       var del_id = $(this).attr('rel');
       $.post('taka_d_cmigratetabledelete.php?del=upd', {delete_id:del_id}, function(data) {
          if(data == 'true') {
	        $('#'+del_id).remove();
			alert('Deleted');
			Count();
