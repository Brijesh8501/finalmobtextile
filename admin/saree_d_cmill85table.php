...
if(isset($_SESSION['User']))
{
	...
	elseif($Design_Id<=0)
		...
		if($action=='insert')
             {
	$query = mysql_query("select * from saree_d_c_2mill where Saree_Lot_Id='".$Saree_Lot_Id."' ") or die(mysql_error());
	$res_Fetch = mysql_fetch_array($query);
	$duplicate = mysql_num_rows($query);
   if($duplicate==0)
    {	
	$query4 = mysql_query("select * from saree_dcorgmill where Saree_Lot_Id='".$Saree_Lot_Id."' ") or die(mysql_error());
	$res_Fetch4 = mysql_fetch_array($query4);
	$duplicate4 = mysql_num_rows($query4);
	$res_Saree_D_C_Id4 = $res_Fetch4['Saree_D_C_Id'];
   if($duplicate4==0)
    {	
	$sql= "INSERT INTO  `saree_d_c_2mill` (`Sa_D_C_Id` ,`Saree_D_C_Id` ,`Saree_Lot_Id` ,`Saree_Lot_Meter` ,`Saree_Weight` ,`Saree_Pieces` ,`Design_Id` ,`R_Id`)VALUES (NULL , '$Saree_D_C_Id' ,  '$Saree_Lot_Id',  '$Saree_Lot_Meter', '$Saree_Weight',  '$Saree_Pieces', '$Design_Id', '$R_Id')";
...
{
$query = mysql_query("select * from saree_dcorgmill where Saree_Lot_Id='".$Saree_Lot_Id."' ") or die(mysql_error());
	$res_Fetch = mysql_fetch_array($query);
	$duplicate = mysql_num_rows($query);
	$res_Saree_D_C_Id = $res_Fetch['Saree_D_C_Id'];
   if($duplicate==0)
    {	
	$sql= "INSERT INTO  `saree_dcorgmill` (`Sa_D_C_Id` ,`Saree_D_C_Id` ,`Saree_Lot_Id` ,`Saree_Lot_Meter` ,`Saree_Weight` ,`Saree_Pieces` ,`Design_Id` ,`R_Id`)VALUES (NULL , '$Saree_D_C_Id' ,  '$Saree_Lot_Id',  '$Saree_Lot_Meter', '$Saree_Weight',  '$Saree_Pieces', '$Design_Id', '$R_Id')";
$result = mysql_query($sql);
	}
	else
	{
...
$query_rsSaree = "SELECT saree_d_c_2mill.Sa_D_C_Id, saree_d_c_2mill.Saree_D_C_Id, saree_d_c_2mill.Saree_Lot_Id, saree_d_c_2mill.Saree_Pieces,saree_d_c_2mill.Saree_Lot_Meter, saree_d_c_2mill.Saree_Weight, quality_details.Quality_Name, design_details.Design, saree_d_c_2mill.R_Id FROM saree_d_c_2mill, quality_details, design_details WHERE design_details.Quality_Id = quality_details.Quality_Id AND saree_d_c_2mill.Design_Id = design_details.Design_Id AND saree_d_c_2mill.Saree_D_C_Id='".$Saree_D_C_Id."' ";
...
{
	mysql_select_db($database_brijesh8510, $brijesh8510);
$query_rsSaree = "SELECT saree_dcorgmill.Sa_D_C_Id, saree_dcorgmill.Saree_D_C_Id, saree_dcorgmill.Saree_Lot_Id, saree_dcorgmill.Saree_Pieces,saree_dcorgmill.Saree_Lot_Meter, saree_dcorgmill.Saree_Weight, quality_details.Quality_Name, design_details.Design, saree_dcorgmill.R_Id FROM saree_dcorgmill, quality_details, design_details WHERE design_details.Quality_Id = quality_details.Quality_Id AND saree_dcorgmill.Design_Id = design_details.Design_Id AND saree_dcorgmill.Saree_D_C_Id='".$Saree_D_C_Id."' ";
$rsSaree = mysql_query($query_rsSaree, $brijesh8510) or die(mysql_error());
$row_rsSaree = mysql_fetch_assoc($rsSaree);
$totalRows_rsSaree = mysql_num_rows($rsSaree);
}
	...
	<?php if($action=='insert')
{
    ...
    $('.btn-danger').click(function(){
       ...
       $.post('saree_d_c_milltabledelete.php?del=ins', {delete_id:del_id}, function(data) {
          if(data == 'true') {
		  $('#'+del_id).remove();
			alert("Deleted!");
			Count();
...
elseif($action=='update')
{
	...
	$('.btn-danger').click(function(){
       var del_id = $(this).attr('rel');
       $.post('saree_d_c_milltabledelete.php?del=upd', {delete_id:del_id}, function(data) {
          if(data == 'true') {
		  $('#'+del_id).remove();
			alert("Deleted!");
			Count();
