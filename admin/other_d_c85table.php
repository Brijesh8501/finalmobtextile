...
if(isset($_SESSION['User']))
{
	...
	elseif($Quantity=="")
		...
		if($action=='insert')
             {
	$query = mysql_query("select * from other_sub_d_c where Mach_Part_Id='".$Mach_Part_Id."' AND Other_D_C_Id = '".$Other_D_C_Id."' ") or die(mysql_error());
	$res_Fetch = mysql_fetch_array($query);
	$duplicate = mysql_num_rows($query);
   if($duplicate==0)
    {	
	$sql= "INSERT INTO  `other_sub_d_c` (`Ot_D_C_Id` ,`Other_D_C_Id` ,`Mach_Part_Id` ,`Quantity` ,`Rate` ,`Amount` ,`R_Id`)VALUES (NULL , '$Other_D_C_Id' , '$Mach_Part_Id', '$Quantity','$Rate','$Amount', '$R_Id')";
...
{
$query = mysql_query("select * from other_sub_orgdc where Mach_Part_Id='".$Mach_Part_Id."' AND Other_D_C_Id = '".$Other_D_C_Id."' ") or die(mysql_error());
	$res_Fetch = mysql_fetch_array($query);
	$duplicate = mysql_num_rows($query);
   if($duplicate==0)
    {	
	$sql= "INSERT INTO  `other_sub_orgdc` (`Ot_D_C_Id` ,`Other_D_C_Id` ,`Mach_Part_Id` ,`Quantity` ,`Rate` ,`Amount` ,`R_Id`)VALUES (NULL , '$Other_D_C_Id' , '$Mach_Part_Id', '$Quantity','$Rate','$Amount','$R_Id')";
$result = mysql_query($sql);
	}
	else
	{
...
$query_rsSaree = "select Ot_D_C_Id,Other_D_C_Id,machine_parts.Mach_Part_Id,machine_parts.Mach_Pname,Quantity,Rate,Amount,R_Id from other_sub_d_c,machine_parts where machine_parts.Mach_Part_Id = other_sub_d_c.Mach_Part_Id AND Other_D_C_Id = '$Other_D_C_Id' ";
...
$(document).ready(function(){
	 var addamount = 0;
                $(".rowDataSd").each(function() {
                    addamount += Number($(this).html());
                });
                $("#Total_Amount").val(addamount.toFixed(2));
	...
	<?php if($action=='insert')
{
    ...
    $('.btn-danger').click(function(){
       ...
       $.post('other_d_c_tabledelete.php?del=ins', {delete_id:del_id}, function(data) {
          if(data == 'true') {
		  $('#'+del_id).remove();
			alert("Deleted!");
			total();
...
elseif($action=='update')
{
	...
	$('.btn-danger').click(function(){
       var del_id = $(this).attr('rel');
       $.post('other_d_c_tabledelete.php?del=upd', {delete_id:del_id}, function(data) {
          if(data == 'true') {
		  $('#'+del_id).remove();
			alert("Deleted!");
			total();
