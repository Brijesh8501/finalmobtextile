...
if(isset($_REQUEST['action']))
{
 ...
 if($action == 'update')
	...
	date_default_timezone_set('Asia/Calcutta');
	 include("webrenew.php");
	$sel_check_entry = "select count(Ta_D_C_Id) as count_taka from taka_dcorgmigrate where Taka_D_C_Id='$Taka_D_C_Id'";
	...
	{
		$msg_check_taka ='<strong style="color:#F00;">'.'* Please update this migration challan which is required</strong>';
	}
}
}
...
{  
	...
	$Taka_Mig = strtoupper($_REQUEST['Taka_Mig']);
	$Status = $_REQUEST['Status'];
	$Entry_Id = $row_result['Registration_Id'];
	date_default_timezone_set('Asia/Calcutta');
	$Entry_Date = date('Y-m-d      h:i:s a');
	$sql= "INSERT INTO  `taka_d_c_migrate` (`Taka_D_C_Id` ,`Taka_D_C_Date` ,`Taka_Mig`,`Total_Taka`,`Entry_Date`,`Entry_Id`,`Status`)VALUES (NULL , '$Taka_D_C_Date' ,  '$Taka_Mig', '$Total_Taka', '$Entry_Date', '$Entry_Id', '$Status')";
$result = mysql_query($sql);
$sel_sub = "select * from taka_d_c_2migrate1 where taka_d_c_2migrate1.Taka_D_C_Id = '$Taka_D_C_Id'";
$sel_exe = mysql_query($sel_sub);
while($sel_fetch = mysql_fetch_array($sel_exe))
{
	$ins_sub = "insert into taka_dcorgmigrate (Ta_D_C_Id,Taka_D_C_Id,Taka_Id,Taka_Meter,Taka_Weight,Quality_Id,R_Id) values(NULL,'".$sel_fetch[1]."','".$sel_fetch[2]."','".$sel_fetch[3]."','".$sel_fetch[4]."','".$sel_fetch[5]."','".$sel_fetch[6]."')";
	mysql_query($ins_sub);
}
$del_sub = "delete from taka_d_c_2migrate1 where taka_d_c_2migrate1.Taka_D_C_Id='$Taka_D_C_Id'";
$del_exe = mysql_query($del_sub);
$sel_sub_or = "select * from taka_dcorgmigrate where taka_dcorgmigrate.Taka_D_C_Id = '$Taka_D_C_Id'";
...
else
{
 ...
 $insertGoTo = "taka_d_c_migratelist?msg_error=$msg_error";
...
else
{
	...
	$sql= "UPDATE `textile`.`taka_d_c_migrate` SET `Taka_D_C_Date` = '$Taka_D_C_Date', `Taka_Mig` = '$Taka_Mig', `Status` = '$Status', `Entry_Date` = '$Entry_Date', `Total_Taka` = '$Total_Taka', `Entry_Id` = '$Entry_Id' WHERE `taka_d_c_migrate`.`Taka_D_C_Id` = '".$Taka_D_C_Id."' ";
$result = mysql_query($sql);
 $updateGoTo = "taka_d_c_migratelist?msg";
        ...
        <div id="div-1" class="accordion-body collapse in body">
            ...
            <div class="form-group">
                    ...
                    <span id="error1" style="color:#F00";></span>
                        <textarea rows="3" cols="20" id="textarea1" name="Taka_Mig" class="form-control" ><?php echo $Taka_Mig;?></textarea>
                    </div>
                </div>
                     </div>
                       <div class="col-lg-4">
	...
	echo "<strong style='color:#F00;'>* Please renew your website</strong>";
                          ...
                          <?php
							if($action=="update" )
                             {
							?>
                                            <a href="taka_d_c_migratelist"><input type="button" value="Back" class="btn btn-inverse btn-lg " name="back"/></a> <input type="submit" value=" Update Challan" class="btn btn-primary btn-lg" name="update" />
                        <?php }
						else if($action=="insert")
						{
						?>
                                  <a href="taka_d_c_migratelist"><input type="button" value="Back" class="btn btn-inverse btn-lg " name="back1"/></a> 
                                   <?php
								   if($days_remaining<=0)
{}
elseif($days_remaining>0)
...
$(document).ready(function(){
		...
		$("#table").load("taka_d_cmigrate85table.php"+sql);
	});
	<?php include("shortcutkeys.php");?>
   </SCRIPT>
<script src="assets/js/takamigrate.js"></script>
...
function getNewChallanID()
{
	...
	$sql = "select max(Taka_D_C_Id)+1 from taka_d_c_migrate";
	$result = mysql_query($sql);
	$row = mysql_fetch_array($result);
	if($row != null && $row[0] > 0)
		{
